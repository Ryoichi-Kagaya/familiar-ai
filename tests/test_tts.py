...
def test_go2rtc_autoconfig_preserves_other_camera_streams(monkeypatch, tmp_path):
    from familiar_agent.tools import tts

    config_path = tmp_path / "go2rtc.yaml"
    config_path.write_text(
        "# Generated by familiar-ai from camera environment settings.\n"
        "streams:\n"
        "  tapo_cam:\n"
        "    - rtsp://primary:secret@192.0.2.10/stream1\n"
    )
    monkeypatch.setattr(tts, "_GO2RTC_CACHE", tmp_path)
    monkeypatch.setattr(tts, "_GO2RTC_CONFIG", config_path)
    monkeypatch.setenv("CAMERA_HOST", "198.51.100.20")
    monkeypatch.setenv("CAMERA_USERNAME", "travel")
    monkeypatch.setenv("CAMERA_PASSWORD", "secret")
    monkeypatch.setenv("CAMERA_TAPO_PASSWORD", "cloud secret")
    monkeypatch.delenv("CAMERA_TAPO_HASH", raising=False)

    tts._write_go2rtc_config("tapo_cam_remote")

    config = tts.yaml.safe_load(config_path.read_text())
    assert set(config["streams"]) == {"tapo_cam", "tapo_cam_remote"}
    assert config["streams"]["tapo_cam"] == ["rtsp://primary:secret@192.0.2.10/stream1"]
    assert config["streams"]["tapo_cam_remote"] == [
        "rtsp://travel:secret@198.51.100.20/stream1",
        "tapo://cloud%20secret@198.51.100.20",
    ]


def test_sync_go2rtc_streams_merges_all_catalog_cameras(monkeypatch, tmp_path):
    from familiar_agent.tools import tts

    config_path = tmp_path / "go2rtc.yaml"
    config_path.write_text("streams:\n  custom: rtsp://example.invalid/stream\n")
    monkeypatch.setattr(tts, "_GO2RTC_CACHE", tmp_path)
    monkeypatch.setattr(tts, "_GO2RTC_CONFIG", config_path)
    monkeypatch.setattr(
        tts.urllib.request,
        "urlopen",
        MagicMock(side_effect=OSError("go2rtc is stopped")),
    )

    tts.sync_go2rtc_streams(
        "http://127.0.0.1:1984",
        {
            "tapo_cam": ["rtsp://main:secret@192.0.2.10/stream1"],
            "tapo_cam_remote": ["rtsp://remote:secret@172.20.10.10/stream1"],
        },
    )

    config = tts.yaml.safe_load(config_path.read_text())
    assert set(config["streams"]) == {"custom", "tapo_cam", "tapo_cam_remote"}


def test_sync_go2rtc_streams_updates_each_running_stream(monkeypatch, tmp_path):
    from familiar_agent.tools import tts

    monkeypatch.setattr(tts, "_GO2RTC_CACHE", tmp_path)
    monkeypatch.setattr(tts, "_GO2RTC_CONFIG", tmp_path / "go2rtc.yaml")
    monkeypatch.setattr(tts.urllib.request, "urlopen", MagicMock())
    upsert = MagicMock()
    monkeypatch.setattr(tts, "_upsert_go2rtc_stream", upsert)
    streams = {
        "tapo_cam": ["rtsp://main:secret@192.0.2.10/stream1"],
        "tapo_cam_remote": ["rtsp://remote:secret@172.20.10.10/stream1"],
    }

    tts.sync_go2rtc_streams("http://127.0.0.1:1984", streams)

    assert upsert.call_count == 2
    upsert.assert_any_call(
        "http://127.0.0.1:1984",
        "tapo_cam_remote",
        streams["tapo_cam_remote"],
    )


def test_ensure_go2rtc_updates_running_process_without_opening_camera(monkeypatch):
    from familiar_agent.tools import tts

    sources = [
        "rtsp://camera:secret@192.0.2.10/stream1",
        "tapo://cloud%20secret@192.0.2.10",
    ]
    write_config = MagicMock(return_value=sources)
    monkeypatch.setattr(tts, "_write_go2rtc_config", write_config)
    responses = []
    ...
    tts._ensure_go2rtc("http://127.0.0.1:1984", "tapo_cam_remote")

    assert responses[0] == "http://127.0.0.1:1984/api"
    update = responses[1]
    assert update.method == "PUT"
    assert "name=tapo_cam_remote" in update.full_url
    assert update.full_url.count("src=") == 2
    assert "192.0.2.10" in update.full_url
    write_config.assert_called_once_with("tapo_cam_remote")


def test_ensure_go2rtc_does_not_start_second_process_when_runtime_update_fails(
    monkeypatch,
):
    from familiar_agent.tools import tts
    ...
    monkeypatch.setattr(
        tts,
        "_write_go2rtc_config",
        MagicMock(return_value=["rtsp://camera:secret@192.0.2.10/stream1"]),
    )
    monkeypatch.setattr(tts, "_upsert_go2rtc_stream", MagicMock(side_effect=OSError("failed")))
    monkeypatch.setattr(tts.urllib.request, "urlopen", MagicMock())
    popen = MagicMock()
