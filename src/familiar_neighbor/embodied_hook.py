...
class EmbodiedAgentHook(RuntimeHookBase):
    ...
    async def prepare_turn(
        ...
        *,
        ...
        if not hasattr(agent, "_schedule_rules"):
            agent._schedule_rules = parse_schedule_config(
        ...
        if not hasattr(agent, "_heartbeat"):
            agent._heartbeat = HeartbeatRuntime(
                ...
                quiet_rules=agent._schedule_rules,
            )
        ...
        turn_max_tokens = (
            min(agent.config.max_tokens, _brief_cap)
            if brief_reply_turn
