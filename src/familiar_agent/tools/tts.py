...
def _write_go2rtc_config(stream_name: str) -> None:
    """Generate go2rtc.yaml from camera vars without clobbering manual setup.
    ...
    # A hand-written config may contain a working protocol-specific
    # backchannel. Keep it unless explicit Tapo credentials ask us to rebuild.
    if _GO2RTC_CONFIG.exists() and not tapo_password and not tapo_hash:
        try:
            existing = _GO2RTC_CONFIG.read_text()
        except OSError:
            existing = ""
        if _GO2RTC_GENERATED_MARKER not in existing:
            logger.debug("Keeping existing go2rtc config (no Tapo autoconfig credentials)")
            return
    ...
    if user and password:
        ...
        entries.append(f"    - rtsp://{rtsp_user}:{rtsp_password}@{host}/{rtsp_path}")
    if tapo_password:
        entries.append(f"    - tapo://{quote(tapo_password, safe='')}@{host}")
    if tapo_hash:
        entries.append(f"    - tapo://admin:{quote(tapo_hash, safe='')}@{host}")
    if not entries:
        return

    yaml_text = (
        f"{_GO2RTC_GENERATED_MARKER}\nstreams:\n  {stream_name}:\n" + "\n".join(entries) + "\n"
    )
