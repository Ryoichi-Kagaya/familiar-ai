...
_GO2RTC_GENERATED_MARKER = "# Generated by familiar-ai from camera environment settings."


class _Go2RTCDumper(yaml.SafeDumper):
    """Emit block sequences indented below their key for go2rtc's YAML patcher."""

    def increase_indent(self, flow: bool = False, indentless: bool = False):
        return super().increase_indent(flow, False)


def _go2rtc_camera_sources() -> list[str]:
    """Build lazy go2rtc sources for the camera in the active environment."""
    ...
    tapo_password = os.environ.get("CAMERA_TAPO_PASSWORD", "").strip()
    tapo_hash = os.environ.get("CAMERA_TAPO_HASH", "").strip()

    sources: list[str] = []
    if user and password:
        rtsp_user = quote(user, safe="")
        rtsp_password = quote(password, safe="")
        sources.append(f"rtsp://{rtsp_user}:{rtsp_password}@{host}/{rtsp_path}")
    if tapo_password:
        sources.append(f"tapo://{quote(tapo_password, safe='')}@{host}")
    if tapo_hash:
        sources.append(f"tapo://admin:{quote(tapo_hash, safe='')}@{host}")
    return sources


def _should_manage_go2rtc_config() -> bool:
    """Return whether camera environment settings own the go2rtc config."""
    tapo_password = os.environ.get("CAMERA_TAPO_PASSWORD", "").strip()
    tapo_hash = os.environ.get("CAMERA_TAPO_HASH", "").strip()
    if not _GO2RTC_CONFIG.exists() or tapo_password or tapo_hash:
        return True
    try:
        return _GO2RTC_GENERATED_MARKER in _GO2RTC_CONFIG.read_text()
    except OSError:
        return False


def _write_go2rtc_config(stream_name: str) -> list[str]:
    """Merge the active camera into go2rtc.yaml and return its lazy sources.

    RTSP is sufficient for video, but many Tapo cameras expose their speaker
    only through go2rtc's ``tapo://`` source. ``CAMERA_TAPO_PASSWORD`` accepts
    the plain cloud password (go2rtc detects the hash method);
    ``CAMERA_TAPO_HASH`` keeps compatibility with pre-hashed credentials.

    Each camera profile owns one named stream. Other streams remain in the
    config, so the primary and travel cameras can coexist in one go2rtc
    process. Declaring a stream does not connect to it; go2rtc opens a source
    only when a consumer requests that stream.
    """
    sources = _go2rtc_camera_sources()
    if not sources or not _should_manage_go2rtc_config():
        return []

    _merge_go2rtc_streams({stream_name: sources})
    logger.info("go2rtc config updated from camera environment (stream=%s)", stream_name)
    return sources


def _merge_go2rtc_streams(stream_updates: dict[str, list[str]]) -> None:
    """Merge named lazy streams into go2rtc.yaml without removing unrelated config."""
    stream_updates = {name: sources for name, sources in stream_updates.items() if sources}
    if not stream_updates:
        return

    config: dict = {}
    if _GO2RTC_CONFIG.exists():
        try:
            loaded = yaml.safe_load(_GO2RTC_CONFIG.read_text())
            if isinstance(loaded, dict):
                config = loaded
        except (OSError, yaml.YAMLError) as exc:
            logger.warning("Could not merge existing go2rtc config: %s", exc)

    streams = config.get("streams")
    if not isinstance(streams, dict):
        streams = {}
        config["streams"] = streams
    streams.update(stream_updates)

    yaml_text = (
        _GO2RTC_GENERATED_MARKER
        + "\n"
        + yaml.dump(
            config,
            Dumper=_Go2RTCDumper,
            sort_keys=False,
            allow_unicode=True,
        )
    )
    _GO2RTC_CACHE.mkdir(parents=True, exist_ok=True)
    _GO2RTC_CONFIG.write_text(yaml_text, encoding="utf-8")
    try:
        _GO2RTC_CONFIG.chmod(0o600)
    except OSError:
        pass
    logger.info("go2rtc config updated with %d named camera stream(s)", len(stream_updates))


def _upsert_go2rtc_stream(api_url: str, stream_name: str, sources: list[str]) -> None:
    """Add or replace one lazy stream in an already-running go2rtc process."""
    query = urlencode([("name", stream_name), *(("src", source) for source in sources)])
    request = urllib.request.Request(f"{api_url}/api/streams?{query}", method="PUT", data=b"")
    with urllib.request.urlopen(request, timeout=5):
        pass


def sync_go2rtc_streams(api_url: str, streams: dict[str, list[str]]) -> None:
    """Persist a camera catalog and update a running go2rtc instance when present.

    Merely declaring these streams is lazy: go2rtc opens a camera connection only
    when something consumes that named stream.
    """
    streams = {name: sources for name, sources in streams.items() if sources}
    if not streams:
        return
    _merge_go2rtc_streams(streams)

    try:
        urllib.request.urlopen(f"{api_url}/api", timeout=2)
    except Exception as exc:
        logger.debug("go2rtc is not running yet; catalog will apply at startup: %s", exc)
        return

    for stream_name, sources in streams.items():
        try:
            _upsert_go2rtc_stream(api_url, stream_name, sources)
        except Exception as exc:
            logger.warning("Could not update running go2rtc stream %s: %s", stream_name, exc)


def _ensure_go2rtc(api_url: str, stream_name: str = "tapo_cam") -> None:
    """Start go2rtc if it's not already running."""
    sources = _write_go2rtc_config(stream_name)

    try:
        urllib.request.urlopen(f"{api_url}/api", timeout=2)
    except Exception as exc:
        logger.debug("go2rtc is not running yet: %s", exc)
    else:
        if sources:
            try:
                _upsert_go2rtc_stream(api_url, stream_name, sources)
    ...
    async def say(self, text: str, output: str | None = None) -> str:
        ...
        async with self._lock:
            ...
            try:
                ...
                # A remote-only request still falls back to the PC speaker.
                # This is intentionally after the go2rtc attempt: synthesis
                # succeeded, so silence is worse than changing output route.
                ...
                route = ", ".join(played_via)
                if output == "remote" and played_via == ["local"]:
                    route = "local fallback"
                return f"Said: {text[:50]}... (via {route})"
            finally:
